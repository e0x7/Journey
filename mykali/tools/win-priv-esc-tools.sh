...
if [ $ip_length -le 5 ]; then
  ...
  if [ ! $ip ]; then
    echo "Selected interface do not have an IP address."
    exit 1
  fi
fi
...
if [ ! -f "Autoruns64.exe" ]; then
  wget https://live.sysinternals.com/Autoruns64.exe
fi
...
echo "certutil.exe -urlcache -split -f http://$ip:$port/Autoruns64.exe Autoruns64.exe"
echo "
For kernel exploit checking CHECK NOTES...make sure smbserver.py is running"
echo ""
